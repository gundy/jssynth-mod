...
import {Envelope} from 'gundy/jssynth';
...
export class Player {
    ...
    handleTick(row, tick, sampleRate) {
        for (var chan = 0; chan < this.song.channels; chan++) {
            ...
            var hasCut = chanState.hasCut;
            var volumeEnvelopeValue = chanState.volumeEnvelopeSequence.next(hasCut).value;
            //if (chanState.hasCut) {
    ...
    handleNote(chan, note, sampleRate) {
        ...
        if (sampleNumber >= 0 && this.song.instruments[sampleNumber]) {
            ...
            parms.volumeEnvelopeSequence = new Envelope(64,
                instrument.volumeType,
                instrument.volumeEnvelope,
                instrument.numVolumePoints,
                instrument.volumeSustainPoint,
                instrument.volumeLoopStartPoint,
                instrument.volumeLoopEndPoint,
                instrument.volumeFadeout
            ).sequence();
        ...
        if (note.note === 254) {  // 254 means note off
            this.mixer.cut(chan);
            parms.hasCut = true;
            console.log("note off on chan "+chan);
        }
        if (volumeEffectHandler) {
        ...
        var hasCut = parms.hasCut;
        var volumeEnvelopeValue = parms.volumeEnvelopeSequence.next(hasCut).value;
